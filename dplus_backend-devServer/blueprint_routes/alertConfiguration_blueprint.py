...
def _local_dev_rows(query, params=None):
    with cso.engine_creation.connect() as conn:
        ...
        for row in result.fetchall():
            rows.append({
                column_name: row_value if not isinstance(row_value, datetime) else row_value.strftime('%d-%m-%Y %H:%M:%S')
                for column_name, row_value in zip(columns, row)
            })
        return {
            "status": 200,
...
def configureAlert(current_user,uniqueId=None):
    ...
    if(request.method=="GET"):
        if getattr(cso, "LOCAL_DEV_MODE", False):
            argu = request.args
            def_start = int(argu.get("start", os.environ.get("DEFAULT_PAGINATION_START", 0)))
            def_end = int(argu.get("end", os.environ.get("DEFAULT_PAGINATION_END", 50)))
            return _local_dev_rows(
                """
                    SELECT
                        ...
                        alertConfig.*
                    FROM alertConfig
                    LEFT JOIN dbConfig ON dbConfig.id = alertConfig.dbServer
                    WHERE IFNULL(alertConfig.deleteStatus, 0) = 0
                      AND alertConfig.createdBy = :createdBy
                    ORDER BY alertConfig.sid
            ...
            return _local_dev_rows(
                """
                    SELECT
                        dbConfig.dbName,
                        alertConfigInstant.*
                    FROM alertConfigInstant
                    LEFT JOIN dbConfig ON dbConfig.id = alertConfigInstant.dbServer
                    WHERE IFNULL(alertConfigInstant.deleteStatus, 0) = 0
                      AND alertConfigInstant.createdBy = :createdBy
                    ORDER BY alertConfigInstant.sid
                    LIMIT :limit OFFSET :offset
                """,
                {
                    "createdBy": current_user["id"],
