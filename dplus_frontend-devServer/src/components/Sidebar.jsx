...
const Sidebar = ({ sidebarOpen }) => {
  ...
  const [openCategory, setOpenCategory] = useState(() => localStorage.getItem(OPEN_CATEGORY_STORAGE_KEY));
  const [activeItem, setActiveItem] = useState(() => localStorage.getItem(ACTIVE_ITEM_STORAGE_KEY) || "Dashboard");
  ...
  useEffect(() => {
    const matchedRouteItem = menu.find((item) => item.type === "link" && item.route === location.pathname);
    if (matchedRouteItem) {
    ...
    <>
      ...
      <aside
        className={`
          ...
          lg:sticky lg:top-0 lg:z-20 lg:h-full lg:min-h-0 lg:self-start lg:translate-x-0 lg:rounded-none lg:border-r-0
      ...
      >
        <div className="flex h-full min-h-0 flex-col items-stretch">
          ...
          <div className="sidebar-scroll flex-1 min-h-0 overflow-y-scroll overflow-x-hidden px-0 pb-3 pt-0 pr-7 lg:pr-7">
