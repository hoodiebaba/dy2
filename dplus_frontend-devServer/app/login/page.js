...
import { isAuthenticated, loginRequest } from "../../lib/portal-auth";
...
export default function LoginPage() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setError(err.message || "Please Use Valid Credentials.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @keyframes cardFloat { 0%,100% { transform: perspective(1400px) rotateX(0deg) rotateY(0deg) translateY(0px); } 25% { transform: perspective(1400px) rotateX(1.3deg) rotateY(-2deg) translateY(-8px); } 75% { transform: perspective(1400px) rotateX(-1.3deg) rotateY(2deg) translateY(6px); } }
        @keyframes cardGlow { 0%,100% { box-shadow: 0 18px 60px rgba(0,0,0,.7); } 50% { box-shadow: 0 24px 80px rgba(0,0,0,.8), 0 0 30px rgba(59,130,246,.08); } }
        @keyframes logoFloat { 0%,100% { transform: translateY(0px) scale(1); filter: drop-shadow(0 0 22px rgba(242,101,34,.22)); } 50% { transform: translateY(-7px) scale(1.03); filter: drop-shadow(0 0 40px rgba(242,101,34,.55)); } }
        @keyframes inputPulse { 0%,100% { box-shadow: 0 0 0 rgba(242,101,34,0); } 50% { box-shadow: 0 0 24px rgba(242,101,34,.12); } }
      ...
      <div className="nebula-bg pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <div className="absolute left-1/2 top-1/2 h-[240vh] w-[240vw] -translate-x-1/2 -translate-y-1/2 opacity-55" style={{ animation: "orbit360 280s linear infinite" }}>
          {mounted && farStars.map((star) => <span key={star.id} className="absolute rounded-full bg-white" style={{ top: star.top, left: star.left, width: `${star.size}px`, height: `${star.size}px`, opacity: star.opacity }} />)}
        </div>
        <div className="absolute left-1/2 top-1/2 h-[240vh] w-[240vw] -translate-x-1/2 -translate-y-1/2" style={{ animation: "orbit360Reverse 190s linear infinite" }}>
          {mounted && midStars.map((star) => <span key={star.id} className="absolute rounded-full bg-[#dbeafe]" style={{ top: star.top, left: star.left, width: `${star.size}px`, height: `${star.size}px`, opacity: star.opacity, boxShadow: "0 0 12px rgba(255,255,255,0.38)", animation: `twinkle ${star.duration} ease-in-out infinite`, animationDelay: star.delay }} />)}
        </div>
        <div className="absolute left-1/2 top-1/2 h-[240vh] w-[240vw] -translate-x-1/2 -translate-y-1/2" style={{ animation: "orbit360 124s linear infinite" }}>
          {mounted && nearStars.map((star) => <span key={star.id} className="absolute rounded-full bg-white" style={{ top: star.top, left: star.left, width: `${star.size}px`, height: `${star.size}px`, opacity: star.opacity, boxShadow: star.size > 4 ? "0 0 24px rgba(255,255,255,0.95)" : "0 0 14px rgba(255,255,255,0.75)", animation: `twinkle ${star.duration} ease-in-out infinite`, animationDelay: star.delay }} />)}
        </div>
        ...
        <div className="absolute inset-0 opacity-[0.86]" style={{ mixBlendMode: "screen" }}>
          <div className="absolute left-[-10%] top-[36%] h-[32rem] w-[32rem] rounded-full" style={{ background: "rgba(162, 28, 175, 0.24)", filter: "blur(130px)" }} />
          <div className="absolute right-[-10%] top-[18%] h-[40rem] w-[40rem] rounded-full" style={{ background: "rgba(37, 99, 235, 0.24)", filter: "blur(150px)" }} />
          <div className="absolute left-[26%] top-[2%] h-[24rem] w-[24rem] rounded-full" style={{ background: "rgba(139, 92, 246, 0.16)", filter: "blur(120px)" }} />
          <div className="absolute bottom-[-12%] left-[34%] h-[28rem] w-[28rem] rounded-full" style={{ background: "rgba(242, 101, 34, 0.12)", filter: "blur(150px)" }} />
        </div>
        ...
        {planets.map((planet) => (
          <div key={planet.id} className="absolute" style={{ top: planet.top, left: planet.left, width: `${planet.orbitSize}px`, height: `${planet.orbitSize}px`, marginLeft: `${planet.orbitSize / -2}px`, marginTop: `${planet.orbitSize / -2}px`, animation: `${planet.reverse ? "orbit360Reverse" : "orbit360"} ${planet.duration} linear infinite`, animationDelay: planet.delay, transformStyle: "preserve-3d" }}>
            ...
            <div className="absolute left-1/2 top-0 -translate-x-1/2" style={{ transform: `translateY(-${planet.size / 2}px)` }}>
              <div className="planet-surface relative rounded-full" style={{ width: `${planet.size}px`, height: `${planet.size}px`, '--planet-color': planet.color, '--spin-duration': `${Math.max(12, planet.size)}s`, boxShadow: planet.glow }}>
                <div className="absolute inset-[18%] rounded-full bg-white/12 blur-[2px]" />
                {planet.ring ? <div className="absolute left-1/2 top-1/2 h-[36%] w-[190%] -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/35" style={{ transform: "translate(-50%, -50%) rotateX(76deg) rotateY(-16deg)", boxShadow: "0 0 12px rgba(255,255,255,0.16)" }} /> : null}
                {planet.moon ? <div className="absolute left-1/2 top-1/2 h-[230%] w-[230%] -translate-x-1/2 -translate-y-1/2" style={{ animation: "moonOrbit 4s linear infinite" }}><div className="absolute right-[-2px] top-1/2 h-[4px] w-[4px] -translate-y-1/2 rounded-full bg-white shadow-[0_0_10px_rgba(255,255,255,0.95)]" /></div> : null}
              </div>
            </div>
          </div>
        ))}
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-[1180px] items-center justify-center">
        <div className="w-full max-w-[540px] rounded-[34px] border border-white/10 bg-[linear-gradient(135deg,rgba(4,7,18,0.92),rgba(12,18,42,0.86))] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.72)] backdrop-blur-2xl sm:p-8" style={{ animation: "cardFloat 10s ease-in-out infinite, cardGlow 8s ease-in-out infinite" }}>
          <div className="mb-6 flex flex-col items-center text-center">
            <img src="/logo.png" alt="Datayog" className="h-16 w-auto" style={{ animation: "logoFloat 7s ease-in-out infinite" }} />
            <h1 className="mt-4 text-[34px] font-extrabold tracking-[0.08em] text-white sm:text-[42px]">
              <span className="text-white">DATA</span><span className="text-[#F26522]">YOG</span>
            </h1>
            <h2 className="mt-3 text-xl font-semibold text-white sm:text-2xl">Sign in to continue</h2>
          </div>

          <form className="space-y-5" onSubmit={handleSubmit}>
            <label className="block">
              <span className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-white/45"><UserCircle2 className="h-4 w-4 text-[#F26522]" /> Username</span>
              <input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Enter username" className="w-full rounded-2xl border border-white/5 bg-[rgba(3,6,18,0.92)] px-4 py-4 text-sm outline-none transition-all duration-200 focus:border-[#F26522]/45" style={{ animation: "inputPulse 5.8s ease-in-out infinite" }} />
            </label>

            <label className="block">
              <span className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-white/45"><LockKeyhole className="h-4 w-4 text-[#F26522]" /> Password</span>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter password" className="w-full rounded-2xl border border-white/5 bg-[rgba(3,6,18,0.92)] px-4 py-4 pr-12 text-sm outline-none transition-all duration-200 focus:border-[#F26522]/45" style={{ animation: "inputPulse 6.4s ease-in-out infinite" }} />
                <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/35 transition hover:text-[#F26522]" aria-label="Toggle password visibility">
            ...
            <button type="submit" disabled={loading} className="w-full rounded-2xl bg-[linear-gradient(135deg,#F26522,#ff8a3d)] px-4 py-4 text-sm font-bold uppercase tracking-[0.22em] text-white transition-all duration-200 hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-70">
